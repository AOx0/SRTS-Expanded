...
public class BomberSkyfaller : Thing, IThingHolder {
  ...
  public override Vector3 DrawPos {
    get {
      ...
      case SkyfallerMovementType.Accelerate:
        return SkyfallerDrawPosUtility.DrawPos_Accelerate(
            base.DrawPos, this.ticksToExit, this.angle, this.speed);
      case SkyfallerMovementType.ConstantSpeed:
        return SkyfallerDrawPosUtility.DrawPos_ConstantSpeed(
            base.DrawPos, this.ticksToExit, this.angle, this.speed);
      case SkyfallerMovementType.Decelerate:
        return SkyfallerDrawPosUtility.DrawPos_Decelerate(
            base.DrawPos, this.ticksToExit, this.angle, this.speed);
      default:
        Log.ErrorOnce("SkyfallerMovementType not handled: " +
                          this.def.skyfaller.movementType,
                      this.thingIDNumber ^ 1948576711, false);
        return SkyfallerDrawPosUtility.DrawPos_Accelerate(
            base.DrawPos, this.ticksToExit, this.angle, this.speed);
      }
    }
  ...
  public IntVec3 DrawPosCell {
    get {
      return new IntVec3((int)this.DrawPos.x, (int)this.DrawPos.y,
                         (int)this.DrawPos.z);
    }
  }
  ...
  private Material ShadowMaterial {
    get {
      if (this.cachedShadowMaterial is null &&
          !this.def.skyfaller.shadow.NullOrEmpty())
        this.cachedShadowMaterial = MaterialPool.MatFrom(
            this.def.skyfaller.shadow, ShaderDatabase.Transparent);
  ...
  public override void ExposeData() {
    base.ExposeData();
    Scribe_Deep.Look<ThingOwner>(ref this.innerContainer, "innerContainer",
                                 new object[] { this });
    ...
    Scribe_Collections.Look<IntVec3>(ref bombCells, "bombCells",
                                     LookMode.Value);
  ...
  public override void SpawnSetup(Map map, bool respawningAfterLoad) {
    ...
    if (!respawningAfterLoad) {
      this.ticksToExit = Mathf.CeilToInt(
          (float)SPExtra.Distance(
              new IntVec3(map.Size.x / 2, map.Size.y, map.Size.z / 2),
              this.Position) *
          2 / this.speed);
    }
    if (sound != null)
  ...
  public override void DrawAt(Vector3 drawLoc, bool flip = false) {
    ...
    this.Graphic.Draw(drawLoc,
                      !flip ? thingForGraphic.Rotation.Opposite
                            : thingForGraphic.Rotation,
  ...
  public override void Tick() {
    ...
    this.ticksToExit--;
    if (bombCells.Any() &&
        Math.Abs(this.DrawPosCell.x - bombCells.First().x) < 3 &&
        Math.Abs(this.DrawPosCell.z - bombCells.First().z) < 3)
      this.DropBomb();
  ...
  private void DropBomb() {
    for (int i = 0;
         i <
         (bombType == BombingType.precise ? this.precisionBombingNumBombs : 1);
         ++i) {
      if (innerContainer.Any(
              x => ((ActiveDropPod)x)
                       ?.Contents.innerContainer.Any(
                           y => SRTSMod.mod.settings.allowedBombs.Contains(
                               y.def.defName)) ??
                   false)) {
        ActiveDropPod srts = (ActiveDropPod)innerContainer.First();

        Thing thing = srts?.Contents.innerContainer.FirstOrDefault(
            y => SRTSMod.mod.settings.allowedBombs.Contains(y.def.defName));
                if (thing is null)
                    return;
                ...
                bombThing.angle =
                    this.angle + (SPTrig.LeftRightOfLine(this.DrawPosCell,
                                                         this.Position, c) *
                                  -10);
                    bombThing.speed =
                        (float)SPExtra.Distance(this.DrawPosCell, c) /
                        bombThing.ticksRemaining;
                    Thing t = GenSpawn.Spawn(bombThing, c, this.Map);
                    GenExplosion.NotifyNearbyPawnsOfDangerousExplosive(
                        t,
                        thing2.TryGetComp<CompExplosive>()
                    ...
                    Block_CEPatched
                    :;
                     // Replaced referencing the projectile trough the
                     // detonateProjectile property of the item's def with
                     // referencing trough AmmoSetDef. The reason is taht not
                     // all mortar shells had detonateProjectile. Don't ask how
                     // I came up with this.
                     ProjectileCE_Explosive CEbombThing =
                         (ProjectileCE_Explosive)ThingMaker.MakeThing(
                             (thing2.def as AmmoDef)
                                 .AmmoSetDefs.Find(set => set.ammoTypes.Any())
                                 .ammoTypes
                                 .Find(link =>
                                           link.ammo == (thing2.def as AmmoDef))
                                 .projectile,
                             null);
                // ProjectileCE_Explosive bombCE =
                // (ProjectileCE_Explosive)ThingMaker.MakeThing((AccessTools.Field(thing2.def.GetType(),
                // "detonateProjectile").GetValue(thing2.def) as ThingDef),
                // null);
                /*ThingComp CEComp = (thing2 as ThingWithComps)?.AllComps.Find(x
                => x.GetType().Name == "CompExplosiveCE"); FallingBombCE
                CEbombThing = new FallingBombCE(thing2, CEComp.props, CEComp,
                this.Map, this.def.skyfaller.shadow); CEbombThing.HitPoints =
                int.MaxValue; CEbombThing.ticksRemaining = timerTickExplode;*/
                IntVec3 c2 = (from x in GenRadial.RadialCellsAround(bombPos, GetCurrentTargetingRadius(), true)
                              where x.InBounds(this.Map)
                              select x).RandomElementByWeight((IntVec3 x) => 1f - Mathf.Min(x.DistanceTo(this.Position) / GetCurrentTargetingRadius(), 1f) + 0.05f);
                /*CEbombThing.angle = this.angle +
                (SPTrig.LeftRightOfLine(this.DrawPosCell, this.Position, c2) *
                -10); CEbombThing.speed =
                (float)SPExtra.Distance(this.DrawPosCell, c2) /
                CEbombThing.ticksRemaining; Thing CEt =
                GenSpawn.Spawn(CEbombThing, c2, this.Map);*/
                GenSpawn.Spawn(CEbombThing, this.DrawPosCell, this.Map);
                    CEbombThing.canTargetSelf = false;
                    CEbombThing.minCollisionDistance = 1;
                    CEbombThing.intendedTarget = null;
                    CEbombThing.AccuracyFactor = 1f; CEbombThing.Launch(
                        this, this.DrawPosCell.ToIntVec2.ToVector2(), 0f,
                        this.angle + UnityEngine.Random.Range(-60f, 60f), 5f,
                        (float)SPExtra.Distance(this.DrawPosCell, c2), this);
                // GenExplosion.NotifyNearbyPawnsOfDangerousExplosive(CEt,
                // DamageDefOf., null); /*Is GenExplosion CE compatible?*/
      }
    }
  ...
  private void ExitMap() {
    ActiveDropPod activeDropPod = (ActiveDropPod)ThingMaker.MakeThing(
        ThingDef.Named(this.def.defName.Split('_')[0] + "_Active"), null);
    activeDropPod.Contents = new ActiveDropPodInfo();
    activeDropPod.Contents.innerContainer.TryAddRangeOrTransfer(
        (IEnumerable<Thing>)((ActiveDropPod)innerContainer.First())
            .Contents.innerContainer,
        true, true);

    TravelingSRTS travelingTransportPods =
        (TravelingSRTS)WorldObjectMaker.MakeWorldObject(
            DefDatabase<WorldObjectDef>.GetNamed("TravelingSRTS", true));
    ...
    travelingTransportPods.arrivalAction =
        new TransportPodsArrivalAction_LandInSpecificCell(
            this.originalMap.Parent, this.sourceLandingSpot);
  ...
  public ThingOwner GetDirectlyHeldThings() { return this.innerContainer; }

  public void GetChildHolders(List<IThingHolder> outChildren) {
    ThingOwnerUtility.AppendThingHoldersFromThings(
        outChildren, this.GetDirectlyHeldThings());
  }
  ...
  private void DrawDropSpotShadow() {
    ...
    Skyfaller.DrawDropSpotShadow(base.DrawPos, base.Rotation, shadowMaterial,
                                 this.def.skyfaller.shadowSize,
                                 this.ticksToExit);
  }

  public static void DrawBombSpotShadow(Vector3 loc, Rot4 rot,
                                        Material material, Vector2 shadowSize,
                                        int ticksToExit) {
    if (rot.IsHorizontal)
    ...
    Graphics.DrawMesh(MeshPool.plane10Back, matrix, material, 0, null, 0,
                      shadowPropertyBlock);
  }
