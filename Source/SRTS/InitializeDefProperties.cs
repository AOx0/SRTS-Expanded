...
internal static class InitializeDefProperties {
  ...
  private static void ModCompatibilityInitialized() {
    ...
    Log.Warning(
        "[SRTS Expanded (CE Fork)] Compatibility with Save our Ship 2 is disabled at the moment. SoS2 compatibility has to happen on the normal SRTS Expanded mod, CE compatibility is included within this fork.");
    foreach (ModMetaData mod in mods) {
      if (ModLister.HasActiveModWithName(mod.Name) &&
          mod.Name == "Combat Extended" && !SRTSHelper.CEModLoaded) {
        Log.Message(
            "[SRTS Expanded (CE Fork)] Initializing Combat Extended patch for Bombing Runs.");
        ...
        SRTSHelper.CompProperties_ExplosiveCE =
            AccessTools.TypeByName("CompProperties_ExplosiveCE");
        SRTSHelper.CompExplosiveCE = AccessTools.TypeByName("CompExplosiveCE");
      }
      /*if(ModLister.HasActiveModWithName(mod.Name) && mod.PackageId ==
      "1909914131" && !SRTSHelper.SOS2ModLoaded)
      {
          Log.Message("[SRTS Expanded] Initializing SoS2 Compatibility Patch.");
          SRTSHelper.SpaceSite =
      DefDatabase<WorldObjectDef>.GetNamed("SiteSpace");
          SRTSHelper.SpaceSiteType = AccessTools.TypeByName("SpaceSite");
          SRTSHelper.SOS2LaunchableType =
      AccessTools.TypeByName("CompShuttleLaunchable"); SRTSHelper.SOS2ModLoaded
      = true;
      }*/
    }
    if (SRTSMod.mod.settings.CEPreviouslyInitialized &&
        !SRTSHelper.CEModLoaded) {
