...
public static class DialogSettings {
  ...
  public static void Settings_IntegerBox(this Listing_Standard lister,
                                         string text, ref int value,
                                         float labelLength, float padding,
  ...
  public static void Settings_Numericbox(this Listing_Standard lister,
                                         string text, ref float value,
                                         float labelLength, float padding,
                                         float min = -1E+09f,
                                         float max = 1E+09f) {
    lister.Gap(12f);
  ...
  public static void Settings_SliderLabeled(this Listing_Standard lister,
                                            string label, string endSymbol,
                                            ref float value, float min,
                                            float max, float multiplier = 1f,
                                            int decimalPlaces = 2,
                                            float endValue = -1f,
    ...
    string format = string.Format(
        "{0}" + endSymbol, Math.Round(value * multiplier, decimalPlaces));
    if (!endValueDisplay.NullOrEmpty() && endValue > 0)
      if (value >= endValue)
        format = endValueDisplay;
    value = Widgets.HorizontalSlider(rect, value, min, max, false, null, label,
                                     format);
  ...
  public static void Settings_SliderLabeled(this Listing_Standard lister,
    ...
    if (!endValueDisplay.NullOrEmpty() && endValue > 0)
      if (value == endValue)
        format = endValueDisplay;
    value = (int)Widgets.HorizontalSlider(rect, value, min, max, false, null,
                                          label, format);
    if (endValue > 0 && value == max)
      value = endValue;
  }
  ...
  public static void
  Settings_Header(this Listing_Standard lister, string header, Color highlight,
  ...
  /// WORK IN PROGRESS
  public static void
  Settings_BoxLabeled(this Listing_Standard lister, Rect rect, string header,
                      Color highlight, GameFont fontSize = GameFont.Small,
                      TextAnchor anchor = TextAnchor.MiddleLeft) {
    var textSize = Text.Font;
    Text.Font = fontSize;
    ...
    Text.Anchor = anchor;
    Rect labelRect =
        lister.GetRect(Text.CalcHeight(header, lister.ColumnWidth));
  ...
  public static bool Settings_Button(this Listing_Standard lister, string label,
                                     Rect rect, Color customColor,
  ...
  public static bool Settings_ButtonLabeled(this Listing_Standard lister,
                                            string header, string buttonLabel,
    ...
    Rect buttonRect =
        new Rect(rect.width - buttonWidth, rect.y, buttonWidth, rect.height);
  ...
  public static void Draw_Label(Rect rect, string label, Color highlight,
                                Color textColor,
                                GameFont fontSize = GameFont.Medium,
                                TextAnchor anchor = TextAnchor.MiddleLeft) {
    var textSize = Text.Font;
  ...
  public static readonly Texture2D ButtonBGAtlas =
      ContentFinder<Texture2D>.Get("UI/Widgets/ButtonBG", true);

  public static readonly Texture2D ButtonBGAtlasMouseover =
      ContentFinder<Texture2D>.Get("UI/Widgets/ButtonBGMouseover", true);

  public static readonly Texture2D ButtonBGAtlasClick =
      ContentFinder<Texture2D>.Get("UI/Widgets/ButtonBGClick", true);
...
public class Dialog_ResearchChange : Window {
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    Rect searchBarRect =
        new Rect(inRect.x, labelRect.y + 24f, inRect.width, 24f);
    ...
    for (int i = 0; i < projectsSearched.Count; i++) {
      ...
      Rect projectRect = new Rect(inRect.x, searchBarRect.y + 24 + (24 * i),
                                  inRect.width, 30f);
      if (Widgets.ButtonText(projectRect, proj.defName, false, false, true) &&
  ...
  public override void PreClose() {
    /*if (addedResearch)
        Messages.Message("RestartGameResearch".Translate(),
       MessageTypeDefOf.CautionInput, false);*/ //Uncomment to send message to restart game if research has been changed
  }
...
public class Dialog_AllowedBombs : Window {
  ...
  public override void DoWindowContents(Rect inRect) {
    Rect labelRect =
        new Rect(inRect.width / 2, inRect.y, inRect.width / 2, 24f);
    Rect label2Rect = new Rect(inRect.x, inRect.y, inRect.width / 2, 24f);
    Rect searchBarRect =
        new Rect(inRect.width / 2, labelRect.y + 24f, inRect.width / 2, 24f);
    ...
    for (int i = 0; i < explosivesSearched.Count; i++) {
      ...
      Rect explosiveRect =
          new Rect(searchBarRect.x, searchBarRect.y + 24 + (24 * i),
                   searchBarRect.width, 30f);
    ...
    Rect outRect =
        new Rect(inRect.x - 6f, searchBarRect.y + BufferArea,
                 searchBarRect.width, inRect.height - (BufferArea * 2));
    Rect viewRect =
        new Rect(outRect.x, outRect.y, outRect.width - 32f,
                 outRect.height *
                     ((float)SRTSMod.mod.settings.allowedBombs.Count / 11f));
    Widgets.BeginScrollView(outRect, ref scrollPosition, viewRect, true);
    for (int i = 0; i < SRTSMod.mod.settings.allowedBombs.Count; i++) {
      string s = SRTSMod.mod.settings.allowedBombs[i];
      Rect allowedExplosivesRect =
          new Rect(inRect.x, searchBarRect.y + 24f + (24f * i),
                   searchBarRect.width, 30f);
