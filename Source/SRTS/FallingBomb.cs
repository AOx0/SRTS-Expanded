...
public class FallingBomb : ThingWithComps {
  ...
  public FallingBomb(Thing reference, CompExplosive comp, Map map,
                     string texPathShadow) {
  ...
  public override void DrawAt(Vector3 drawLoc, bool flip = false) {
    ...
    this.Graphic.Draw(drawLoc, !flip ? base.Rotation.Opposite : base.Rotation,
                      this as Thing, angleDropped);
    this.DrawDropSpotShadow();
  }

  public override Vector3 DrawPos {
    get {
      return FallingBomb.DrawBombFalling(base.DrawPos, this.ticksRemaining,
    ...
    get {
      if (texPathShadow != null && !texPathShadow.NullOrEmpty())
        cachedShadowMaterial =
            MaterialPool.MatFrom(texPathShadow, ShaderDatabase.Transparent);
  ...
  protected void DrawDropSpotShadow() {
    if (this.ShadowMaterial is null)
      return;
    FallingBomb.DrawDropSpotShadow(
        base.DrawPos, base.Rotation, ShadowMaterial,
        new Vector2(this.RotatedSize.x, this.RotatedSize.z), ticksRemaining);
  }

  protected static void DrawDropSpotShadow(Vector3 center, Rot4 rot,
                                           Material material,
                                           Vector2 shadowSize,
                                           int ticksToImpact) {
    if (rot.IsHorizontal)
    ...
    Graphics.DrawMesh(MeshPool.plane10Back, matrix, material, 0, null, 0,
                      FallingBomb.shadowPropertyBlock);
  }
  ...
  protected virtual void ExplodeOnImpact() {
    if (!this.SpawnedOrAnyParentSpawned)
      return;
    if (props.destroyThingOnExplosionSize <= explosionRadius &&
        !this.Destroyed) {
      this.Kill(null, null);
    }
    if (props.explosionEffect != null) {
      Effecter effecter = props.explosionEffect.Spawn();
      effecter.Trigger(new TargetInfo(this.PositionHeld, map, false),
                       new TargetInfo(this.PositionHeld, map, false));
      effecter.Cleanup();
    }
    if (def.projectileWhenLoaded?.projectile != null) {
      GenExplosion.DoExplosion(
          this.PositionHeld, map, explosionRadius, projectile.damageDef, this,
          projectile.GetDamageAmount(1), projectile.GetArmorPenetration(1),
          projectile.soundExplode, null, def.projectileWhenLoaded, null,
          projectile.postExplosionSpawnThingDef,
          projectile.postExplosionSpawnChance,
          projectile.postExplosionSpawnThingCount,
          projectile.applyDamageToExplosionCellsNeighbors,
          projectile.preExplosionSpawnThingDef,
          projectile.preExplosionSpawnChance,
          projectile.preExplosionSpawnThingCount,
          projectile.explosionChanceToStartFire,
          projectile.explosionDamageFalloff);
    } else {
      GenExplosion.DoExplosion(
          this.PositionHeld, map, explosionRadius, props.explosiveDamageType,
          this, props.damageAmountBase, props.armorPenetrationBase,
  ...
  public static Vector3 DrawBombFalling(Vector3 center, int ticksToImpact,
  ...
  protected static MaterialPropertyBlock shadowPropertyBlock =
